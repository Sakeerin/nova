...
3. **A char literal inside a `${…}` interpolation hole does not lex** either (`"${'a' == 'b'}"` is `L0001`). Fixture and test programs must bind such a comparison to a local first and interpolate the local — worth knowing before writing S10's fixture, where it would otherwise look like a bug in the string code.
