...
**Add a second section to this ADR, for a decision made during implementation:** why `Iterator`'s four consumers take plain `self` rather than `mut self`.
  ...
  - **What this gives up in ADR 0005 — do not write "it routes around the rule rather than weakening it."** That was this plan's original wording and it is too strong; it was retracted in `f7a1308`, which appended an amendment to ADR 0005 itself. **Read `docs/adr/0005-mutable-receivers-and-one-shot-hash.md`'s amendment before writing this bullet and do not state anything weaker than it does.** The distinction it draws: the *mechanism* is intact (`mut self` still rejects a temporary on an inherent method, a trait method, and `next` itself — all three measured `E0060`), but the *property* ADR 0005 advertised is partly given up. Its Consequences promised "every std API that mutates must declare `mut self` — an accessor that forgets it will not compile," and four shipped consumers now mutate, declare plain `self`, and compile.
  - **Why the collateral loss was unavoidable, and name it.** `place_root` returns three verdicts — `Mutable`, `ImmutableLocal(name)`, `NotAPlace` — and `mut self` rejects the latter two (`check.rs:5772-5789` and `require_mutable_place` at `:5799-5831`). Only `NotAPlace`, the temporary, blocked the advertised chain. Losing `ImmutableLocal` was **collateral**, because Nova has no third receiver form that accepts a temporary while still rejecting an immutable local. The observable consequence, which the ADR must show:
    ...
    State that this loss is real and was not the goal.
  ...
  - **What bounds the loss** — three things, all from the ADR 0005 amendment: `next` stays `mut self`, so driving an iterator by hand still needs a mutable binding; **no new iterator state becomes reachable**, since a `mut` binding could always be partially consumed and reused; and the compiler already launders a mutation this way in its own `for` desugar (`check_for_iterator` binds `__it` mutable regardless of the source expression's mutability), so `Iterator`'s consumers are the first *library* use of a pre-existing escape hatch, not a new one.
  - **Consequence to state plainly.** A consumer now accepts a temporary, so `v.iter().count()` compiles — that is the intent. It also accepts an immutable local and advances it silently, which is not. Both follow from the same relaxation and only the first was wanted.
