...
This plan is kept as written and amended here rather than corrected in place: a plan rewritten to match what shipped would erase the evidence that it was wrong, which is what a superseded plan is still good for. **Several items below are not statements but instructions**, so flagging them is not enough — an instruction stays executable until something tells an executor not to execute it.

**The instructions flagged below are Step 3, Step 6, Step 8 and Task 2 Step 2 — those four among however many this plan holds, and the roster is OPEN, not closed.** It was a closed list of one until the fix round that closed this increment found three more in this same file. A reader who finds a fifth should expect it to be real: this note's silence about a step is not clearance for that step, and the failure mode that left all four standing — a correction reaching the code and the records but not the plan that dictated them — has no reason to have stopped at four. Verify a step against the shipped tree and the design record before executing it.
...
**THREE MORE INSTRUCTIONS MUST NOT BE FOLLOWED AS WRITTEN.** Added at the fix round, later than everything above, which named only Step 3. Each is identified by its heading, quoted far enough to be unmistakable, and paired with what happened instead.

**Step 6, headed "Run mutation 2 — break phase 1's grouping", must not be run for the result it predicts.** It directs the executor to change `if counts[b] >= WANT` to `if counts[b] >= 1` in `PHASE1_SRC` and predicts "**FAIL** at the `count 32` assertion". **That prediction is false: the mutation escapes the test, and running it would record a live mutant as a dead one.** With the threshold at 1, phase 1's search loop breaks on its first candidate and `target` becomes whatever bucket `"k0"` landed in — but the *emitting* loop below it is untouched, and it rescans the whole 4000-candidate budget for keys in that bucket. An arbitrary bucket expects 62.5 of those 4000, so the mutant fails only if fewer than 32 arrive: `P(Bin(4000, 1/64) <= 31)` is 6.97e-06, about one run in 143,000, and conditioning on the first candidate already being in that bucket — which the mutation guarantees — drops it further, to about one in 298,000. Both figures computed with exact rationals. Worse, the set the mutant hands phase 2 genuinely collides, so `concentration 32` and phase 2's bound behave exactly as they do unmutated. **What was run instead:** a mutation making the emitting loop's filter always true, so phase 1 emits the first 32 candidates regardless of bucket. That drove the self-check's `concentration` to 2 and failed the `concentration 32` assertion for a reason that is forced rather than probabilistic.

**Step 8, headed "Run mutation 4 — drop the finalizer, keep the seed. EXPECTED TO PASS.", must not be run for the conclusion it states, and its heading is wrong.** It predicts "Expected: **PASS**. This is not a defect and must not be 'fixed'. A seed change alone moves these keys, so this test cannot see the finalizer". **This increment's own measured figures falsify both the prediction and the reason.** Without the finalizer the six bucket-selecting bits are a function of `seed & 63` alone, so the layout space collapses from 2^64 to 4096 residue pairs; enumerated over all 4096 against the real hash and the real key strings, **416 of them fail the bound** — about one run in ten — and **256 of them print `largest bucket 32 of 32`**, the same output a wholly dead seed produces. So the mutation is a coin-toss that fails about a tenth of the time, not an expected pass, and a single observed pass would be recording noise. Those figures and the enumeration method are in the shipped test's own doc comment. **The usable statement survives in stronger form, and it is what to record instead:** coverage of the finalizer here is partial and unreliable, `tests/runtime/hash_diffusion.nova` is the deterministic detector, and a failure of the precomputation test must never be read as a dead seed without checking the finalizer separately.

**Task 2 Step 2, headed "Point the two gate claims at the executing test", must not be followed in full — the worst of the four named here, because it directs an overclaim into two normative records.** It instructs the executor to add, to `nova-spec/20-STDLIB.md` section 7 and `docs/adr/0018-std-json-scope-and-build-order.md`, "that the adaptive limit is demonstrated by that test's own first phase". **Do not write that into either record.** Phase 1 observes no timing: it calls `.hash()` directly, which requires code running inside the target process and is a stronger capability than observing timing from outside, so it is not an instance of the adversary those records exclude. **What happened instead:** the executor declined that half and wrote the disclaimer positively, at the commit whose subject is "point both gate claims at the executing test, scope unmoved" — so neither record carries the overclaim, and both now state that the timing exclusion gains nothing from this increment. Following the step as written now would put both records in contradiction with their own text. Step 2's other half is right and was followed: the precomputing half is now pinned by `hashdos_precomputed_key_set_does_not_survive_a_new_process`, and the gate's scope is unchanged. The same overclaim also reached this plan's dictated Phase 1 doc comment and the design record's section 3; the shipped comment was corrected on this branch, at the commit whose subject is "stop the hashdos test's comment claiming it demonstrates the timing limit", and the design record carries the narrowing in its section 11c.
...
**Amended 2026-08-28: the sentence above is false as shipped, and the table it closes is short.** The increment also amended `docs/superpowers/specs/2026-08-26-map-hashdos-design.md`, `docs/adr/0005-mutable-receivers-and-one-shot-hash.md`, its own governing spec and this plan itself. Its claim about `std/` did hold -- no file under `std/` changed. What it got wrong was quantifying over everything else, which is the same bare-closed-world shape the "Sentence-shape discipline" section below forbids -- not Global Constraints, which holds the build, test, lint and byte-scan rules; an earlier wording of this note cited the wrong section.

A later increment corrected the stale byte-access wording in `nova-runtime`, `nova-resolver` **and `std/core/lib.nova`**. That last one matters here and an earlier wording of this note omitted it: it is a file under `std/`, so a reader who takes the paragraph above as standing guidance would be misled twice over -- the prohibition was this plan's, scoped to its own increment, and a later increment did edit a `std/` file. What held is narrower than the sentence: no file under `std/` changed in THIS increment.
