...
- **Line endings are per file: check each file's line endings before editing it; never infer a directory convention.** Every file *this* increment touched happens to be CRLF (`std/sync/lib.nova`, every fixture and golden, `CHANGELOG.md`, `docs/adr/0017-…`, `nova-spec/20-STDLIB.md`), but an earlier version of this constraint generalised that to "all markdown under `docs/` and `nova-spec/`", and **that is false**: measured 2026-08-22, 20 of the 75 tracked markdown files under those two directories are LF-only, and `20-STDLIB.md` is the *only* CRLF file in all of `nova-spec/` — `00-MASTER-SPEC.md`, `12-TYPESYSTEM.md`, `13-RUNTIME.md` and `14-CODEGEN.md` are all LF-only. This matters concretely for the increment ADR 0017 routes next: `13-RUNTIME.md` is LF-only, and the generalised rule applied there would convert all 239 of its lines. Verify `tr -cd '\r' < f | wc -c` against `wc -l < f` before and after each edit. **`grep` here strips CR from its output**, so `cat -A` shows no `^M` even when present — `od -c` or a byte count is authoritative. **`sed -i` silently stripped all 166 CRs from a `.nova` file last increment**, caught only by a byte count. Prefer the Edit tool. **CORRECTED 2026-08-22 (branch `spec-line-endings-correction`): the specific measurements above are a WORKTREE SNAPSHOT, not a property of the files, and the alarm they raise is misdirected.** Measured exhaustively on `86aefcd`: **0 of 85 tracked `.md` files carry CR in the committed blob** — `20-STDLIB.md` included, despite reading CRLF in the worktree. With `core.autocrlf=true` and no `.gitattributes`, git normalises every text file to LF on the way in, so worktree CRLF-ness is only ever whatever your last checkout produced. `13-RUNTIME.md` is the demonstration: LF (CR=0) throughout the increment that rewrote it, CRLF (CR=454) an hour later after a fast-forward re-checkout, **blob LF both times**. So the warning that editing it "would convert all 239 of its lines" was wrong twice over — the conversion is invisible to `git diff` and harmless to the repo, because the blob was going to be LF either way. That is also why the `sed -i` incident showed nothing in `git diff --stat`: normalisation working, not damage hidden. **Keep the byte count as a tripwire for *did my tool rewrite this file wholesale*, which is a real hazard, but stop treating a line-ending change as repo corruption and stop asserting that any directory has a line-ending convention.**
...
| `tests/runtime/channel_*.nova` + `.stdout` (create, 6 pairs) | One fixture per behaviour. The harness reads each golden with `.replace("\r\n", "\n")`, so it works whatever the worktree representation is — see the line-endings constraint above, corrected 2026-08-22. |
