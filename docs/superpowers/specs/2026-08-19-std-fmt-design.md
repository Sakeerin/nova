...
- **Format specifications in interpolation syntax** (`${x:>8.2}`). Measured: this does not parse today — `error[P0001]: expected '}' (in string interpolation), found ':'`. Adding it is a **lexer** change, not a stdlib one, and it carries a real ambiguity: interpolation holes are lexed **inline**, their tokens going straight into the stream (`crates/nova-lexer/src/lib.rs:7`), and a colon is **already legal inside a hole** — `"${P { x: 3 }}"` compiles and prints `P(3)`, measured. So `:` cannot simply mean "begin format spec"; the lexer would need a rule at `:` that consults the per-hole brace counter it already has (`crates/nova-lexer/src/lib.rs:9-11`) — the counter exists, only the rule reading it at `:` does not. That is a smaller gap than a from-scratch brace-depth mechanism, tractable, and not justified yet: nothing in the tree wants aligned interpolation badly enough to pay for a grammar change.
- **`Formatter`.** §3 declares `pub record Formatter { ... }` with its body elided and describes it only as a "Format builder for Display impls". There is no specified behaviour to build. `Display` is `fn fmt(self) -> String` (`std/core/lib.nova:98`) and returns a whole string in one call regardless, so any builder reachable from it is a longer, slower spelling of interpolation: a `mut self` accumulator (ADR 0005 §1, ten uses in shipped `std/collections`) is buildable inside `fmt` without changing `Display`'s shape at all, but each append copies the string accumulated so far, making it quadratic where interpolation is one lowering. Not built because there is nothing specified to implement and the alternative is strictly worse — not because the language lacks the means. Deferred until something needs it, and §3 amended to say so.
...
| Mutation | Caught by |
|---|---|
| `pad`'s width comparison `<` → `<=` | **Equivalent mutant — nothing can catch it.** At `len == width`, `width - len` is `0`, so `"0".repeat(0)` is `""` on both the `<` and the `<=` reading and the two arms produce identical output. `fmt_int_pad`'s own fixture table above (§8, the table two rows up) has no equal-width row, and the shipped fixture has none either — there is nothing to name here. (This row previously named `fmt_int_pad`'s "equal-width row" as the catcher; the plan had already measured this mutant equivalent (`plans/2026-08-19-std-fmt.md:274`) before this spec was written, and §7's own rule — *"after retracting a claim, grep the document for the claim's consequences"* (§7, this file, above) — applied to the plan's retraction but not to this table, which is exactly the kind of surviving corollary that rule exists to catch.) |
