...
**The `checked_add` overflow fallback, and what it does and does not guarantee.** `Instant + Duration` panics on overflow and is reached from `poll_sleep`, across which no panic may pass, so `instant_from_deadline_nanos` uses `checked_add` with a halving fallback (`furthest_representable_instant`). **That fallback's guarantee — that it never answers with something `wake_due` treats as already due — is empirical, not structural**, and the implementation says so at length; an earlier draft of that comment stated a proof that does not hold ("never earlier than `base`, and `base` is in the past" permits "earlier than `now`", which is the already-due case; the real lower bound is `base + headroom/2`). What holds is that headroom from a process-start `base` vastly exceeds uptime on every supported backend, and that the fallback is unreachable through this crate's API at all, since `deadline_nanos_from_now` saturates at `i64::MAX` ns. **Clamping the result forward (`max(candidate, now + 1ns)`) to make it structural is rejected, not merely unchosen:** it reintroduces the very livelock the fallback replaced, because `poll_sleep` compares the stored *integer* deadline — astronomically large in the overflow case — so a `now + 1ns` instant wakes the task a nanosecond later, finds itself not due, and re-stages forever. It also puts a panicking `Instant + Duration` back on a path no panic may cross.
...
- **An empty record parses.** `record Marker {}` and `Marker {}` both type-check, so `TimeoutError` can carry nothing — which §9 specifies, and which the caller does not need since it passed `d` in. It is the one record in `std/time` needing no field-privacy disclosure, because it has no field — not the first in `std`, which `std/io`'s fieldless `Stdin`/`Stdout`/`Stderr` already were. The in-source comment scopes itself to "this library" and is correct as written; only this line and `CHANGELOG.md`'s claimed the wider superlative.
...
- **The discriminating one:** `timeout(d, f)` where `f` produces `42` must print `42`.

  **This bullet has now been corrected twice; the second correction was itself wrong. Everything below was measured by running it, and nothing below is inferred.** The history is worth keeping, because the failure mode is the recurring one on this project — asserting what a mutant *would* do instead of running it:

  1. As first written, it claimed the mutation was "reading `task_output` on the timeout's own future instead of the inner's", which prints the status `0`. **False:** that does not compile. `task_output`'s builtin signature is `(vec![Future<Param(0)>], Param(0))`; inside `timeout<T>`'s own body, `Param(0)` is `T` held rigid, checked once, and `unify` has no arm joining a rigid `Ty::Param` to a concrete type. The combinator's own future is `Future<Int>` — `task_timeout_future`'s builtin return type is concrete, never `Future<Param(0)>` — so binding it to `task_output`'s `Future<T>` parameter fails typeck with `E0010` before the mutant runs. This part stands, verified twice independently.
  2. The correction then named three mutations that "would each compile cleanly and still print the status `0`". Its third — `task_timeout_future`'s Rust-side signature widened to return `Future<Param(0)>` — **also does not compile**, and is blocked by the rigid-`Param` rule the same paragraph had just established two sentences earlier: widening `builtin_signature`'s `Builtin::TaskTimeoutFuture` return to `Ty::Future(Box::new(Ty::Param(0)))` yields `error[E0010]: mismatched operand types: T0 vs Int` at `<std/time>:181:8`, because `Param(0)` binds to the rigid `T` from `fut: Future<T>` and then cannot meet `TIMEOUT_COMPLETED: Int`. Deleted.
  3. Of the two survivors, **only one is actually pinned by this fixture alone**, which the correction also got wrong:
     - **A wrong runtime slot layout** — measured by making `poll_timeout` also write its status into the *inner's* output slot: compiles clean, prints `0` instead of `42`, and across the whole workspace **`timeout_value_run` is the only test that fails**. This is the fixture's real, exclusive discriminating power.
     - **A `nova-mir` lowering bug in `Builtin::TaskOutput`** — measured by making `Lowering::FutureOutput` load `STATE_SLOT_TAG` instead of `STATE_SLOT_OUTPUT`: also compiles clean, and also prints `0` instead of `42`, so the "compiles cleanly and prints `0`" half is true. But it is **not** uniquely caught: it fails every test that reads a value through that lowering — `block_on`, `join`, and `nova test`'s own async bodies — which measured as **eight** failures in `nova-cli::run_tests`, all on the async path, `timeout_value_run` among them. **Do not quote a precise count here.** Three measurements of this same mutation gave 18, 7 and 8: the first was taken while an unrelated test-infra defect was live in the tree and swept 11 non-async `*_build_standalone` tests in with it, and the remaining two differ by one test across runs. The count is not the finding; the shape is. A bug in this lowering is broadly detected, and this fixture is not what stands between the project and it.

  So the accurate claim, and the only one this fixture's own header should make, is the narrow one: **a runtime slot-layout bug in `poll_timeout` is caught here and nowhere else.**
