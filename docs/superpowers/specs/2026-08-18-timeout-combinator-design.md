...
- **`task_output(fut: Future<T>) -> T` already exists**, typed `(vec![future_of_param0()], Ty::Param(0))`, and reads "the value a finished future wrote to its state object's output slot". So the inner's value is read **from the inner's own slot** and never moves — which removes any interaction with the output slot's scalar/pointer non-discrimination.
...
- **The discriminating one:** `timeout(d, f)` where `f` produces `42` must print `42`. A mutant reading the timeout's own output slot instead of the inner's prints the status `0`, so this is what pins `task_output(fut)` reading the **inner** future's slot.
