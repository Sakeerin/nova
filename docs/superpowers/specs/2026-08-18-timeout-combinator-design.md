...
- **The discriminating one:** `timeout(d, f)` where `f` produces `42` must print `42`.
  ...
  **This bullet has now been corrected twice; the second correction was itself wrong. Everything below was measured by running it, and nothing below is inferred.** The history is worth keeping, because the failure mode is the recurring one on this project — asserting what a mutant *would* do instead of running it:
  ...
  3. Of the two survivors, **only one is actually pinned by this fixture alone**, which the correction also got wrong:
     ...
     - **A `nova-mir` lowering bug in `Builtin::TaskOutput`** — measured by making `Lowering::FutureOutput` load `STATE_SLOT_TAG` instead of `STATE_SLOT_OUTPUT`: also compiles clean, and also prints `0` instead of `42`, so the "compiles cleanly and prints `0`" half is true. But it is **not** uniquely caught: **18** tests in `nova-cli::run_tests` fail under it, `timeout_value_run` among them. That is unsurprising in hindsight — `Builtin::TaskOutput` is what `block_on` and `join` read every value through — so any bug in its lowering is broadly detected, and this fixture is not what stands between the project and it.
